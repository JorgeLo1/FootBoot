...
from config.settings import (
    ...
    # API_FOOTBALL_KEY eliminada — ESPN no requiere key
)
...
def _format_nacional_message(bets_df: pd.DataFrame,
    ...
    lines = [
        ...
        "_Eliminatorias · Copa América · Mundial_",
...
def run_calendar():
    ...
    log.info("=" * 55)
    try:
        from src.espn_collector import run_daily          # ← ESPN
        result = run_daily()
        log.info(f"Calendar OK: {len(result.get('fixtures', []))} partidos hoy")
...
def run_predict():
    ...
    from src.espn_collector import (                      # ← ESPN
    ...
    client = ApifootballClient()                          # sin key, sin check
    ...
    try:
        if _should_retrain(historical):
            ...
            if len(training_df) >= 100:
                ...
                log.warning(
                    f"Solo {len(training_df)} partidos históricos. "
                    "Usando modelo de clubes como base."
                )
            ...
            if _has_models():
                ...
                log.warning("Sin modelo nacional. Usando modelo de clubes.")
                dc, ensemble = load_models()
...
def _train_nacional(training_df: pd.DataFrame):
    ...
    from src._03_model_engine import DixonColesEnsemble, FootbotEnsemble
    ...
    engine_mod.FEATURE_COLS = [c for c in NACIONAL_FEATURE_COLS
...
def _patch_feature_cols(ensemble):
    pass  # el patch real ocurre en _train_nacional
