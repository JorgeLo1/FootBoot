...
Cliente Supabase v2 (fix pydantic incompatibility)
...
def guardar_prediccion(
    ...
    try:
        ...
        logger.error("Error guardando predicción %s vs %s [%s]: %s", home_team, away_team, mercado, e)
        return None
...
def cerrar_prediccion(
    ...
    """
    Inserta el resultado real de una predicción.
...
def obtener_predicciones_abiertas(fecha: Optional[date] = None) -> list[dict]:
    """
    Retorna predicciones que aún no tienen resultado registrado.
    Si se pasa fecha, filtra por esa fecha.
    """
    supabase = get_client()

    try:
        # Predicciones que NO tienen fila en resultados
        query = (
        ...
        if fecha:
            query = query.eq("fecha", str(fecha))
...
def test_conexion() -> bool:
    ...
    try:
        client = get_client()
        # Query simple que no falla aunque las tablas no existan
